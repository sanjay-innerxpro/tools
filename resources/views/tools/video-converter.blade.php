...
<div x-data="videoConverter()" x-cloak class="max-w-4xl mx-auto px-4 py-10">
    <div class="text-center mb-8">
        <div class="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-red-500 to-orange-500 rounded-2xl shadow-lg shadow-red-500/20 mb-5">
            <svg class="w-8 h-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
        </div>
        <h1 class="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white">{{ __('Video Converter') }}</h1>
        <p class="text-gray-500 dark:text-gray-400 mt-2 text-lg">{{ __('Convert videos between popular formats quickly') }}</p>
    </div>
    ...
    <div x-show="!result" class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-800 p-8 mb-6">
        <div @dragover.prevent="drag=true" @dragleave.prevent="drag=false" @drop.prevent="drag=false;dropFile($event)"
            ...
            <input type="file" x-ref="fi" @change="pickFile($event)" accept="video/*,.mkv,.flv,.wmv,.3gp,.ts" class="hidden">
            <div x-show="!file" class="space-y-3">
                <div class="w-14 h-14 bg-gray-100 dark:bg-gray-800 rounded-xl flex items-center justify-center mx-auto">
                    <svg class="w-7 h-7 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                </div>
                <p class="text-gray-600 dark:text-gray-300 font-medium">{{ __('Drop your video here or click to browse') }}</p>
                <p class="text-sm text-gray-400">{{ __('MP4, WebM, AVI, MOV, MKV, FLV — Max 200 MB') }}</p>
            </div>
        ...
        <div class="mt-6">
            ...
            <div class="flex flex-wrap gap-2">
                <template x-for="f in formats" :key="f">
                    <button @click="format=f" :class="format===f?'bg-red-600 text-white border-red-600':'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:border-red-400'" class="px-4 py-2 text-sm font-medium border rounded-lg transition-all uppercase" x-text="f"></button>
                </template>
            </div>
        </div>

        <div class="mt-6 flex justify-center">
            <button @click="run()" :disabled="!file||busy" class="px-8 py-3 bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 disabled:from-gray-400 disabled:to-gray-400 text-white font-semibold rounded-xl shadow-lg transition-all flex items-center gap-2">
                <template x-if="!busy"><span class="flex items-center gap-2"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>{{ __('Convert Video') }}</span></template>
    ...
    <template x-if="result">
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-800 p-6 fade-in">
            ...
            <div class="mt-4 text-center"><button @click="reset()" class="text-sm text-blue-600 dark:text-blue-400 hover:underline">{{ __('Convert another video') }}</button></div>
        </div>
    </template>
</div>
@endsection
...
@push('scripts')
<script>
function videoConverter(){return{file:null,format:'mp4',formats:['mp4','webm','avi','mov','mkv'],busy:false,error:null,result:null,drag:false,
dropFile(e){const f=e.dataTransfer.files[0];if(f&&(f.type.startsWith('video/')||/\.(mkv|flv|wmv|3gp|ts|avi)$/i.test(f.name))){this.file=f;this.error=null}else this.error='{{ __("Please drop a video file.") }}'},
